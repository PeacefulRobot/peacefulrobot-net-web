<!DOCTYPE html><html><head><!-- link(rel="stylesheet" href="/main.css")--></head><body><p><a href="https://travis-ci.org/isaacs/node-glob/"><img src="https://travis-ci.org/isaacs/node-glob.svg?branch=master" alt="Build Status"></a> <a href="https://david-dm.org/isaacs/node-glob"><img src="https://david-dm.org/isaacs/node-glob.svg" alt="Dependency Status"></a> <a href="https://david-dm.org/isaacs/node-glob#info=devDependencies"><img src="https://david-dm.org/isaacs/node-glob/dev-status.svg" alt="devDependency Status"></a> <a href="https://david-dm.org/isaacs/node-glob#info=optionalDependencies"><img src="https://david-dm.org/isaacs/node-glob/optional-status.svg" alt="optionalDependency Status"></a></p>
<h1>Glob</h1><p>Match files using the patterns the shell uses, like stars and stuff.</p>
...
<h2>Usage</h2><pre><code class="language-javascript">var glob = require(&quot;glob&quot;)
...
<h2>Glob Primer</h2><p>&quot;Globs&quot; are the patterns you type when you do stuff like <code>ls *.js</code> on
...
<h3>Dots</h3><p>If a file or directory path portion has a <code>.</code> as the first character,
...
<h3>Basename Matching</h3><p>If you set <code>matchBase:true</code> in the options, and the pattern has no
slashes in it, then it will seek for any file anywhere in the tree
with a matching basename.  For example, <code>*.js</code> would match
<code>test/simple/basic.js</code>.</p>
<h3>Negation</h3><p>The intent for negation would be for a pattern starting with <code>!</code> to
match everything that <em>doesn&#39;t</em> match the supplied pattern.  However,
the implementation is weird, and for the time being, this should be
avoided.  The behavior is deprecated in version 5, and will be removed
entirely in version 6.</p>
<h3>Empty Sets</h3><p>If no matching files are found, then an empty array is returned.  This
...
<h2>glob.hasMagic(pattern, [options])</h2><p>Returns <code>true</code> if there are any special characters in the pattern, and
...
options.</p>
<h2>glob(pattern, [options], cb)</h2><ul>
...
<h2>glob.sync(pattern, [options])</h2><ul>
<li><code>pattern</code> {String} Pattern to be matched</li>
<li><code>options</code> {Object}</li>
<li>return: {Array<string>} filenames found matching the pattern</string></li>
</ul>
<p>Perform a synchronous glob search.</p>
<h2>Class: glob.Glob</h2><p>Create a Glob object by instantiating the <code>glob.Glob</code> class.</p>
...
immediately.</p>
<h3>new glob.Glob(pattern, [options], [cb])</h3><ul>
...
<h3>Options</h3><p>All the options that can be passed to Minimatch can also be passed to
...
<li><code>ignore</code> Add a pattern or an array of patterns to exclude matches.</li>
...
broken symlink)</li>
<li><code>nonegate</code> Suppress deprecated <code>negate</code> behavior.  (See below.)
Default=true</li>
<li><code>nocomment</code> Suppress deprecated <code>comment</code> behavior.  (See below.)
Default=true</li>
</ul>
<h2>Comparisons to other fnmatch/glob implementations</h2><p>While strict compliance with the existing standards is a worthwhile
...
<h3>Comments and Negation</h3><p><strong>Note</strong>: In version 5 of this module, negation and comments are
<strong>disabled</strong> by default.  You can explicitly set <code>nonegate:false</code> or
<code>nocomment:false</code> to re-enable them.  They are going away entirely in
version 6.</p>
<p>The intent for negation would be for a pattern starting with <code>!</code> to
match everything that <em>doesn&#39;t</em> match the supplied pattern.  However,
the implementation is weird.  It is better to use the <code>ignore</code> option
to set a pattern or set of patterns to exclude from matches.  If you
want the &quot;everything except <em>x</em>&quot; type of behavior, you can use <code>**</code> as
the main pattern, and set an <code>ignore</code> for the things to exclude.</p>
<p>The comments feature is added in minimatch, primarily to more easily
support use cases like ignore files, where a <code>#</code> at the start of a
line makes the pattern &quot;empty&quot;.  However, in the context of a
straightforward filesystem globber, &quot;comments&quot; don&#39;t make much sense.</p>
<h2>Windows</h2><p><strong>Please only use forward-slashes in glob expressions.</strong></p>
...
<h2>Race Conditions</h2><p>Glob searching, by its very nature, is susceptible to race conditions,
...
<h2>Contributing</h2><p>Any change to behavior (including bugfixes) must come with a test.</p>
